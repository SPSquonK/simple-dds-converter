...
class BytesScanner {
    ...
    nextDWORD() {
        if (!this.hasXBytesLeft(4)) throw Error("Unexpected EOF");
...
function decomposeType(typeStr) {
    ...
    if (bracket === -1) return [typeStr, null];
...
function readDDSHeader(buffer) {
    ...
    if (dds.dwMagic !== 542327876) throw Error('Bad value for dwMagic');
